...
def get_iid_class_and_labels(original_images, original_labels, N):
    M = len(original_labels) // N
    clients_images = [[] for _ in range(N)]
    clients_labels = [[] for _ in range(N)]
    indices_list = [i for i in range(len(original_labels))]
    for i in range(N):
        while(len(clients_labels[i]) < M):
            random_indice = random.randint(0, len(indices_list)-1)
            clients_images[i].append(original_images[indices_list[random_indice]])
            clients_labels[i].append(original_labels[indices_list[random_indice]])
            indices_list.pop(random_indice)
...
for i in range(client_number):
    ...
    save_data(client,'./data/cifar10-c-feature-only/{}.pkl'.format(i))
...
save_data(client_1,'./data/cifar10-c-feature-only/test-1.pkl')
# save_data(client_2,'./data/cifar10-c-concept-only/test-2.pkl')
# save_data(client_3,'./data/cifar10-c-concept-only/test-3.pkl')
