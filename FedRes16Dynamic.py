...
log_file = './ResNet/FedDynamic/cifar10/label/Res-label-training.log'
logging.basicConfig(filename=log_file, level=logging.INFO, format='%(asctime)s [%(levelname)s] - %(message)s')
writer = SummaryWriter(log_dir='./Res16/FedDynamic/cifar100/label/')
...
def feddynamic_aggregation(global_model, client_models, client_performance, base_aggregation_weight=0.1):
    """
    Aggregates the client models using FedDynamic algorithm.
    :param global_model: The global model.
    :param client_models: List of client models.
    :param client_performance: A list of client performances which influences the aggregation.
    :param base_aggregation_weight: Base aggregation weight to ensure all clients contribute.
    :return: Updated global model.
    """
    global_dict = global_model.state_dict()
    total_performance = sum(client_performance)

    # Calculate aggregation weights for each client based on performance
    aggregation_weights = [base_aggregation_weight + performance / total_performance for performance in
                           client_performance]

    # Normalize aggregation weights
    weight_sum = sum(aggregation_weights)
    normalized_weights = [weight / weight_sum for weight in aggregation_weights]

    # Before calculating aggregation weights, ensure client_performance is not empty and has valid values
    if not client_performance or sum(client_performance) == 0:
        raise ValueError("Client performance list is empty or invalid")

    # Aggregate the models
    for key in global_dict.keys():
        global_dict[key] = torch.stack(
            [client_models[i].state_dict()[key].float() * normalized_weights[i] for i in range(len(client_models))], 0
        ).sum(0)
    global_model.load_state_dict(global_dict)

    # Optionally, you can update the client models with the new global model weights
...
def main():
    ...
    # 数据加载和预处理
    data_dir = "./data/cifar10-c-label-only/"
    batch_size = 32
    num_epochs = 50
    ...
    start_epoch = 0
    checkpoint_path = "./ResNet/FedDynamic/cifar10/label/checkpoint_epoch_40.pth"
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for client_id in range(num_clients):
            ...
            # Assuming the performance is related to client_accuracy in this case
            client_performances.append(client_accuracy)  # Add performance metric to the list

            # Collect local model weights
            client_weights.append(copy.deepcopy(local_model.state_dict()))

        # Ensure the list is not empty before aggregation
        if not client_performances:
            raise ValueError("Client performances list is empty, cannot perform aggregation.")

        global_model = feddynamic_aggregation(global_model, client_models, client_performances)
        ...
        if (epoch + 1) % 5 == 0 or epoch == num_epochs - 1:
            checkpoint_path = f"./ResNet/FedDynamic/cifar10/label/checkpoint_epoch_{epoch + 1}.pth"
            torch.save({
                'epoch': epoch + 1,
